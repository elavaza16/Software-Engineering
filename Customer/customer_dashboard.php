...
// FIX: Path changed to correctly reference the file in the parent directory.
...
function fetch_customer_data($db, $user_id) {
    ...
    // Fetch all available cities for the profile update form
        ...
        if ($stmt = mysqli_prepare($db, $sql_garage_search)) {
            ...
            if (mysqli_stmt_execute($stmt)) {
                ...
                while ($row = mysqli_fetch_assoc($result)) {
                    ...
                    // Assign path retrieved directly from SQL
...
$all_cities = $customer_data['all_cities']; // Fetched cities list
